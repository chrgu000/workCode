 ...
 */
@Service
public final class OrgaInfoEventHandler implements IDmpEventHandler<OrgaInfoEvent> {
	
	private static Logger logger = org.slf4j.LoggerFactory.getLogger(OrgaInfoEventHandler.class);
	...
	@Transactional
	@Override
	public void onEvent(OrgaInfoEvent event, long sequence, boolean endOfBatch)
			throws Exception {
		
		Map<String,List<TbOrgaMsgVo>> listMap = Maps.newHashMap();
		...
		if (StringUtils.isEmpty(result)) { // 返回为空表明正常
			...
			listMap.put("0", (List<TbOrgaMsgVo>) event.getData());
		}else {
			listMap.put("1", (List<TbOrgaMsgVo>) event.getData());
		}
